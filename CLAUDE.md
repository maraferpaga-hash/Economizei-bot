...
**Última atualização:** 2026-06-07 (gráfico do `/gastos` trocado de rosca para barras horizontais em `src/charts.js` — leitura melhor, mesma assinatura, sem migration. Pendências anteriores seguem: correção dos outputs `migration_2026-06-07_coerencia_outputs.sql`; assinatura MP migration + ativação; indicação `/convidar` migration 003)
