...
3. **Máquina commita SÓ em branch `maquina/*` — `main` e `push` são do Gabriel** *(revisado 2026-08-05; era "máquina nunca commita" desde 2026-06-24)* — mudou a **redação**, não o espírito: o gate real sempre foi o push (que deploya no Railway), e esse não se moveu. A máquina cria `maquina/cod-XXXX`, commita ali e para. **Nunca:** `git push`, `git merge`, `git rebase`, `reset --hard`, commit na `main`, `branch -D`, force-push, mexer em tags/remotes. Merge/push/deploy/migrations = sempre Gabriel, via `/entregar` (aprovação dupla: `npm run check` verde **no resultado do merge** + "APROVO" literal; checagem de migrations/envs BLOQUEANTE antes do push). Regem a pilha as 3 Leis da Máquina 3.0 (linear · teto 3 · main parada) — ver AGENDA "📚 Pilha da máquina".
