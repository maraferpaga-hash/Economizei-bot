...
- ⚠️ **Em aberto (teste de 5 min enfileirado):** o fix do lock **invalidou a premissa** que criou a variante TREE (*"o sandbox não consegue commitar"*) — o lock vinha da **leitura**, não da escrita. Nunca se testou de verdade se o sandbox commita. Nos dois resultados o ganho é o mesmo: **um modo só, decidido por evidência**.
...
## 6. Estado atual (retrato de 2026-08-09)
...
- **`origin/main` = HEAD = `92dd273`.** Entregas recentes: cod-0043 (`9c094aa`) · cod-0068/0067/0025 + Máquina 3.0 + corpus real (até `b485ba8`) · **cod-0044 + cod-0048** (`41beafe`, reconciliado em `92dd273`).
- **🔴 Esteira entupida — item nº 1 do projeto hoje:** a **cod-0062a** (blindagem de agregação — filtro de `tipo`/`direcao` explícito em toda leitura de gasto) está **implementada e não commitada desde 2026-08-07**. Arquivos: `src/supabase.js` (+224/−54) e `test/filtro-gasto.test.js` (novo, 18 testes, todos verdes). **Sem migration, sem env nova, nenhum número muda hoje.** Já bloqueou as runs de **08/08 e 09/08** (produção da máquina em zero há 2 dias) e bloqueará a de amanhã. A AGENDA **está correta** desta vez (cod-0062a em "🔧 Em revisão"). Um `/entregar` em modo TREE resolve.
- **Fila da máquina (reabastecida 08-07 por FATIAMENTO, não por tarefa nova):** o problema não era falta de item, era **composição** — quase tudo era porte G (coração). Fatiadas pelo critério "toca `src/gemini.js`?": **cod-0062a** (feita, esperando commit) · **cod-0062b** (guard `precos_mercado` + copy PIX) · **cod-0065a** (módulo puro de datas, 4 formatos do corpus) · **cod-0065b** (`fmtMoeda`) · **cod-0072a** (parser de parcela). Mais **cod-0073/0074/0075** (gate Pro) e **cod-0066** (limpeza MP, liberada em 08-07 após arbitragem da contradição). O `coerceNumber` ficou **de propósito** com o Gabriel — coerção numérica afeta todo cupom brasileiro.
- **🔴 Pendências humanas quentes (ordem de urgência):** (1) **`/entregar` a cod-0062a** — destrava a máquina; (2) **sentada única no SQL Editor** (`Roteiro_SQL_Editor_2026-08-07.md`, ~25 min): passo 0 = provar no WhatsApp que a `service_role` está em uso → migration PIX → **RLS (2 scripts)** → S3 → DROP MP; (3) substituir `.claude/commands/tarefa.md` pelo `tarefa_CORRIGIDO_2026-08-07.md`; (4) setar `COMPARATIVO_MAX_PRO=10`; (5) teste de 5 min do commit no sandbox (decide Máquina 3.0 × TREE).
...
- **Esteira entupida** — `.js` não commitado ⇒ a run matinal não implementa nada. Destrava com `/entregar`.
...
*Retrato de 2026-08-09 (gerado pela sentinela semanal — substitui o de 2026-08-03). Em caso de conflito com os arquivos da pasta `C:\Economizei`, a pasta vence. Atualizar a cada mudança estrutural (novo pilar, mudança de modelo de negócio, virada de fase).*
