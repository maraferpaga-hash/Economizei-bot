...
**🔐 Estado (2026-08-18, sessão de fechamento):** **S4 FECHADO — o RLS está ligado** (os 2 scripts rodados). Encerra a exposição dos dados via anon key e derruba o bloqueio das cod-0069/0070. **cod-0073 (gate Pro) pronta pra `/entregar`** — no working tree desde 16/08, 461/469 testes verdes no sandbox (as 8 falhas são **SIGBUS do `sharp`**, limitação do ambiente pela regra 11 — o gate real é o `npm run check` na sua máquina; o teste novo `comparativo-gate` passa 11/11 e nenhum teste do formatter regrediu, confirmando a retrocompatibilidade). **Segue aberto no banco:** S3 (RPC), S5 (views de métricas), migration PIX, DROP MP.
...
### 🔧 cod-0073 · Gate Pro no `/comparar` — **no WORKING TREE, sem commit → `/entregar` em modo TREE**
- **status:** em-revisao · **data:** 2026-08-16 (rotina matinal, variante SANDBOX — a máquina não commita aqui)
- **⚠️ FINANCEIRO — commit consciente.** O firewall acusa **de propósito** (`temFeaturesProAtivas`, `ehPro`, `COMPARATIVO_MAX_PRO`, `/planos`, `is_pro`, `features_pro_ate`) e retorna **exit 0** (advisory desde 2026-07-26). É o desenho da tarefa, não um erro.
- **mapa tarefa→arquivos:**
  - `src/index.js` — import de `temFeaturesProAtivas`; `mostrarComparativo(phone, usuario)`; teto por plano
  - `src/formatter.js` — `montarMensagemComparativo(resultado, opts = {})` + rodapé de upsell
  - `src/supabase.js` — **só comentário** (o helper `temFeaturesProAtivas` já existia; atualizada a nota que dizia "quando essas features forem implementadas")
  - `test/comparativo-gate.test.js` — **novo**, 11 testes
- **migration necessária:** ❌ **não** — nenhuma coluna/tabela nova; `is_pro` e `features_pro_ate` já existem e já vêm no `select` do `upsertUsuario`.
- **efeito:** fecha o achado **B10 🔴** do Checkpoint N2 de 01/08 — o comparativo deixa de ser idêntico pra Free e Pro, e a recompensa de indicação (`features_pro_ate`) passa a destravar algo real.
- **pré-req humano (não bloqueia o merge, só a visibilidade):** setar `COMPARATIVO_MAX_PRO=10` no Railway e no `.env.example` (zona do Gabriel). Sem a env o default do código já é 10 — **não quebra nada**.
- **fora-de-escopo respeitado:** cod-0074 (gate no Alerta Pro) e cod-0075 (gate na intent do Agente) **não** foram tocadas — `src/agent/intents.js:596` segue usando `COMPARATIVO_AMOSTRAS_FREE` pra todos, que é exatamente o vazamento que a cod-0075 fecha. **Enquanto a cod-0075 não subir, o Pro é gated no `/comparar` e destravado ao perguntar em texto livre.**
- **skills usadas:** economizei-code-decisions, economizei-tdd, economizei-financial-firewall, economizei-copywriter, copy-review, economizei-product-principles
